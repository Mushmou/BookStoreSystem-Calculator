...
class BinaryTree:
    ...
    def in_order(self):
        u = self.r
        if u != self.nil:
            self.in_order_helper(u)

    def in_order_helper(self, u : Node):
        if u.left is not self.nil:
            self.in_order_helper(u.left)
        print(u.x,":", u.v)
        if u.right is not self.nil:
            self.in_order_helper(u.right)
            

    def pre_order(self) :
        u = self.r
        if u != self.nil:
            self.pre_order_helper(u)
        
    def pre_order_helper(self, u : Node):
        print(u.x,":", u.v)
        if u.left is not self.nil:
            self.pre_order_helper(u.left)
        if u.right is not self.nil:
            self.pre_order_helper(u.right)


    def post_order(self) :
        u = self.r
        if u != self.nil:
            self.post_order_helper(u)
    
    def post_order_helper(self, u : Node):
        if u.left is not self.nil:
            self.post_order_helper(u.left)
        if u.right is not self.nil:
            self.post_order_helper(u.right)
        print(u.x,":", u.v)
